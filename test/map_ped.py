...
data=pd.read_csv("data/BK.csv",low_memory=False)
data.head()
...
with open('data/BK.json') as data_file:    
...
for prop in data['features']:
	...
	prop['properties'][u'ped_energy'] = str(ped).strip('[]')

with open('data/osm_ped.json', 'w') as outfile:
